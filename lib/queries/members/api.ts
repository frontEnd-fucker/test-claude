...
import { getUsersByIds, getUserByEmail } from "@/lib/queries/users/api";

/**
 * Fetch project members with user information
 * Includes defensive fallback for project owner if not in members table
 */
export async function fetchProjectMembers(
  ...
  if (userIds.length === 0) {
    return formattedMembers; // Early return if no members
  }
...
export async function addProjectMember(
  ...
  // If email is provided but userId is not, try to find user by email
  ...
  const user = await getUsersByIds([targetUserId]);
